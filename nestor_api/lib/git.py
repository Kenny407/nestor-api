...
def branch(repository_dir, branch_name):
    ...
    stdout = io.execute(f"echo $(git branch -a | grep {branch_name})", repository_dir)
...
def create_working_repository(app_name):
    ...
    Logger.debug(
        ...
        "[repositories-manager] Created a working repository",
    )
...
def get_last_tag(repository_dir):
    """Retrieves the last tag of a repository"""
...
def get_remote_url(repository_dir, remote_name="origin"):
    ...
    return io.execute(f"git config --get remote.{remote_name}.url", repository_dir)
...
def get_last_commit_hash(repository_dir):
    """Retrieves the last commit hash of a repository"""
...
def tag(repository_dir, app_name, tag_name):
    ...
    app_config = config.for_app(app_name)
...
def update_pristine_repository(app_name):
    ...
    Logger.debug(
        {app_name, repository_dir},
        "[repositories-manager] Updated a pristine repository",
    )
...
def update_repository(repository_dir, git_url, revision="origin/master"):
    ...
    if io.exists(repository_dir):
        ...
        if remote_url == git_url:
            # If the remote are the same, clean and fetch
            ...
            # If the remote mismatch, remove the old one
