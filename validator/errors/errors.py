...
class InvalidTargetPathError(Exception):
    """Exception raised when the target path (NESTOR_TARGET_PATH) env is not set"""

    def __init__(self, target_path: str):
        super().__init__(
            f"The environment variable NESTOR_TARGET_PATH is not set. Current value: ${target_path}"
        )
